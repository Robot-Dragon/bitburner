...
export async function main(ns) {
	...
	while ((ns.cloud.getServerCost(ram_amount) * 25) > current_money) {
		ram_amount = ram_amount / 2
	}

	// Purchase servers with appropriate RAM
	for (let i = 0; i < 25; ++i) {
		ns.cloud.purchaseServer('purchased-server', ram_amount);
	}
